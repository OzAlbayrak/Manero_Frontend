	...
	return (
		<div className='container payment-method p-0'>
			<Header
				...
				hasCart={false}
			></Header>
			<div className=' mx-auto payment-method-container'>
						...
						{Array.isArray(creditCards) && creditCards.length > 0 ? (
							creditCards.map((creditcard, index) => (
								<div className='item-container ' key={creditcard.creditCardId}>
									<img className='card-image' src={images[index % 2]} alt='card' />
				...
				<PaymentMethodField
					...
					submitbutton={
						<i className='fa-light fa-pen-line fa-xs' style={{ color: 'black' }}></i>
					}
				/>
				<PaymentMethodField
					title={'Pay Pal'}
				...
				<PaymentMethodField
					title={'Payoneer'}
					submitbutton={
						<i className='fa-sharp fa-light fa-plus' style={{ color: 'black' }}></i>
					}
				/>
			</div>
		</div>
